		...
		<script>
			...
			function repopulateForm(){
				...
				%>
					...
					$("textarea[name='city']").val('${sessionScope.member.city}');
					$("textarea[name='district']").val('${sessionScope.member.district}');
					$("textarea[name='address']").val('${sessionScope.member.address}');
					$(".gender[value='<%= member.getGender()%>']").prop('checked', true);
					$("input[name='phoneNumber']").val('<%= member.getPhoneNumber()%>');					
		...
		--%>			
			...
			<p>
				...
				<input type='password' id='pwd1' name='password1' placeholder="輸入新密碼" disabled 	 
					maxlength="<%= Member.MAX_PASSWORD_LENGTH %>" minlength="<%= Member.MIN_PASSWORD_LENGTH %>"><br>
				<label>確認: </label>
				<input type='password' id='pwd2' name='password2' placeholder="輸入確認密碼" disabled 
					maxlength="<%= Member.MAX_PASSWORD_LENGTH %>" minlength="<%= Member.MIN_PASSWORD_LENGTH %>">
